...
int HashInsert(HashTable* Q3HashPtr, int key)
{
    //write your code here
    int i;
    // need to use the hashsearch function to determine whether the key is already in the hash table
    ...
    newNode->key = key;
    newNode->next = NULL;
    // need to remember to increment nsize which is the number of inserted keys and also the headptr of the hashnode
    if (temp == NULL)
    {
        Q3HashPtr->Table[i].head = newNode;
        Q3HashPtr->Table[i].size++;
        Q3HashPtr->nSize++;
        return 1;
    }
