...
int recurBST(BTNode *root, BTNode* left, BTNode *right)
{
    if(root==NULL)
    {
        return 1;
    ...
    return recurBST(root->left, left, root) && recurBST(root->right, root, right);
}
...
int isBST(BTNode *root)
{
    ...
    return recurBST(root, NULL, NULL); 
}
...
int nodeNum(BTNode *root)
{
    ...
    {
        count += nodeNum(root->left);
        count += nodeNum(root->right);
        return count;
    }

}
BTNode* findBST(BTNode *root)
{
    ...
    BTNode *left = findBST(root->left);
    BTNode *right = findBST(root->right);
...
BTNode* findLargestBST (BTNode* root)
{
    ...
    {
        BTNode *leftBST = findBST(root->left);
        BTNode *rightBST = findBST(root->right);

        if (nodeNum(leftBST) > nodeNum(rightBST))
        {
            return leftBST;
        }
        else if (nodeNum(leftBST) < nodeNum(rightBST)) 
        {
            return rightBST;
        }
        else 
